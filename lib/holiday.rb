...
def capitalize_key(title)
  string_title = title.to_s
  title_array = string_title.split("_")
  title_array.each do |word|
      word.capitalize!
  end
  capitalized_title = title_array.join(" ")
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, contents|
    cap_season = capitalize_key(season)
    puts "#{cap_season}:"
    holiday_hash[season].each do |holiday, supplies|
      cap_holiday = capitalize_key(holiday)
      current_supplies = supplies.join(", ")
      puts "  #{cap_holiday}: #{current_supplies}"
    end
  end
end
